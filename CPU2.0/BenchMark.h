...
class BenchMark {
 public:
  ...
  void HHHyperBench_simd(uint32_t MEMORY, double alpha, uint32_t HASH_NUM = 2,
                    std::string BENCHNAME = "SPAO") {
    benchname = BENCHNAME;

    memory = MEMORY;

    std::cout << "start HyperBench simd!" << std::endl;

//    HASH_NUM = 5;

    OurHyper_simd *sketch = new OurHyper_simd(MEMORY, HASH_NUM);
    ...
    std::cout << "Hyper_simd Insert Time: " << tm_insert.count() / 1000 << "s"
              << std::endl;
    std::cout << "Hyper_simd Throughput: " << throughput << std::endl;
    ...
    std::cout << "Hyper_simd Query Time: " << tm_query.count() / 1000 << "s"
              << std::endl;
  ...
  void HHHyperBench_murmurhash(uint32_t MEMORY, double alpha, uint32_t HASH_NUM = 2,
                    std::string BENCHNAME = "HSSO") {
    benchname = BENCHNAME;

    memory = MEMORY;

    std::cout << "start HyperBench murmurhash!" << std::endl;

//    HASH_NUM = 5;

    OurHyper_murmurhash *sketch = new OurHyper_murmurhash(MEMORY, HASH_NUM);
    ...
    std::chrono::duration<double, std::milli> tm_insert =
        end_insert - start_insert;
    throughput = length / (tm_insert.count() / 1000);
    std::cout << "Hyper_murmurhash Insert Time: " << tm_insert.count() / 1000 << "s"
              << std::endl;
    std::cout << "Hyper_murmurhash Throughput: " << throughput << std::endl;
    ...
    std::chrono::duration<double, std::milli> tm_query =
        end_query - start_query;
    std::cout << "Hyper_murmurhash Query Time: " << tm_query.count() / 1000 << "s"
              << std::endl;
  ...
  void HHHyperBench_precalculate(uint32_t MEMORY, double alpha, uint32_t HASH_NUM = 2,
                    std::string BENCHNAME = "PLNO") {
    benchname = BENCHNAME;

    memory = MEMORY;

    std::cout << "start HyperBench precalculate!" << std::endl;

//    HASH_NUM = 5;

    OurHyper_precalculate *sketch = new OurHyper_precalculate(MEMORY, HASH_NUM);
    ...
    std::chrono::duration<double, std::milli> tm_insert =
        end_insert - start_insert;
    throughput = length / (tm_insert.count() / 1000);
    std::cout << "Hyper_precalculate Insert Time: " << tm_insert.count() / 1000 << "s"
              << std::endl;
    std::cout << "Hyper_precalculate Throughput: " << throughput << std::endl;
    ...
    std::chrono::duration<double, std::milli> tm_query =
        end_query - start_query;
    std::cout << "Hyper_precalculate Query Time: " << tm_query.count() / 1000 << "s"
              << std::endl;
 ...
 private:
  ...
  void Compare_FullKey(T mp, T record, double alpha) {
    ...
    double aae_sum = 0.0;
    double are_sum = 0.0;
    double aae_fra_sum = 0.0;
    double are_fra_sum = 0.0;
#ifndef AVERAGE
    ...
    for (int i = 0; i < TUPLES_VALUES_ELEMENT_NUM; ++i) {
	  ...
	  int j = (i+1)%TUPLES_VALUES_ELEMENT_NUM;
	  double realFd = it->second.values[j];
          double estFd = mp[it->first].values[j];
          bool reald = (abs(realFd) > threshold);
          bool estd = (abs(estFd) > threshold);
	  if (reald && estd && realFd != 0 && estFd != 0) {
            fracHH += 1;
            aae_fra += abs(realF/realFd - estF/estFd);
            are_fra += abs(realF/realFd - estF/estFd) / abs(realF/realFd);
	  }
        }
      }
      if (bothHH != 0) {
      ...
      if (fracHH != 0) {
        aae_fra_sum += aae_fra / fracHH;
        are_fra_sum += are_fra / fracHH;
      } 
    }
#else
    ...
    ofs << benchname << " " << memory << " " << precision_sum / NUM << " "
        ...
        << are_sum / NUM << " ";
#ifndef AVERAGE
    ofs << aae_fra_sum / NUM << " " << are_fra_sum / NUM << " ";
#endif
  ...
  void Compare_PartialKey(T mp, T record, double alpha) {
	...
	double aae_fra = 0, are_fra = 0;
	    ...
	    int j = (i+1) % TUPLES_VALUES_ELEMENT_NUM;
            double realFd = it->second.values[j];
            double estFd = tmp_mp[it->first].values[j];
            bool reald = (abs(realFd) > threshold);
            bool estd = (abs(estFd) > threshold);
            if (reald && estd && realFd != 0 && estFd != 0) {
              fracHH += 1;
